...
class PackGpk(Command):
    ...
    def run(self):
        ...
        if sys.platform != "linux2" and sys.platform.find("win") == -1:
            raise OSError("arc4nix only supports Windows and Linux.")

        # Now we need copy exec_file.py to the gpk folder then pack it
        import shutil
        shutil.copy(
            src=os.path.join("arc4nix", "exec_file.py"),
        ...
        shutil.copy("ExecFile_v101.gpk", os.path.join("arc4nix", "data"))
        pass
...
setup(
    ...
    url='https://github.com/striges/arc4nix',
    license='MIT',
    author='sugar',
